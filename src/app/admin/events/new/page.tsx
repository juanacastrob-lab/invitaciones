  ...
  return (
    <AdminShell me={me} title="Nuevo evento" current="/admin/events">
      ...
      <div className="rounded-sm border border-stone-200 bg-white p-5">
        <EventBasicsForm initial={{ slug: '', type: 'boda', packageCode: '', template: 'aurora', partnerA: '', partnerB: '', startsAt: '', timezone: 'America/Mexico_City', country: 'MX', languages: ['es'], defaultLanguage: 'es', rsvpDeadline: '', allowPublicRsvp: false, showPrivateGifts: true, checkinEnabled: false }} packages={packages.filter((p) => p.active)} />
      </div>
    </AdminShell>
  );
}
