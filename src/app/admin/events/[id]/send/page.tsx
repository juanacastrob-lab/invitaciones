...
export default async function SendPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AdminShell me={me} title={eventNames(c.couple)} current="/admin/events">
      ...
      <SendQueue eventId={id} slug={event.slug} siteUrl={site} couple={eventNames(c.couple)} startsAt={c.startsAt} timezone={event.timezone} guests={guests} templates={templates} />
    </AdminShell>
  );
}
